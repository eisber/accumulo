...
public class AvroRowEncoderIteratorTest {
  ...
  private void validateSingleRowSimpleSchema(SortedMap<Key,Value> map, MyRow... expectedRows)
      throws IOException {
    ...
    Schema schema = SchemaBuilder.record("root").fields().name("cf1")
        .type(SchemaBuilder.record("cf1").fields().optionalString("cq1").endRecord()).noDefault()
        .endRecord();
