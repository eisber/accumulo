...
import org.apache.accumulo.core.iterators.user.avro.juel.AvroUtf8Wrapper;
...
public class AvroFastRecord implements GenericContainer, IndexedRecord {
  ...
  public AvroFastRecord(Schema schema) {
    ...
    this.nestedFields =
        schema.getFields().stream().filter(f -> f.schema().getType() == Type.RECORD).map(f -> {
    ...
    this.primitiveFields = schema.getFields().stream()
        .filter(f -> f.schema().getType() != Type.RECORD).mapToInt(f -> f.pos()).toArray();
  }
  ...
  /**
   * Create the core lookup map for column family/column qualifier. The leave nodes are consumers
   * that know which record/field to target.
   * 
   * @param rootRecord
   *          the root Avro record.
   * @return the lookup map.
   */
  public static Map<ByteSequence,Map<ByteSequence,RowBuilderCellConsumer>>
      createCellToFieldMap(AvroFastRecord rootRecord) {
    ...
    for (Field field : rootRecord.getSchema().getFields()) {
      ...
      Map<ByteSequence,RowBuilderCellConsumer> nestedLookupMap = nestedSchema.getFields().stream()
          .collect(Collectors.toMap(
              ...
              nestedField -> createAvroCellConsumer((AvroFastRecord) rootRecord.get(field.pos()),
                  nestedField)));
  ...
  /**
   * Creates a consumer of cells that copy the data into the corresponding Avro record fields.
   * 
   * @param record
  ...
  private static RowBuilderCellConsumer createAvroCellConsumer(AvroFastRecord record, Field field) {
    ...
    Encoder<?> encoder =
        RowBuilderType.valueOf(field.getProp(AvroSchemaBuilder.ROWBUILDERTYPE_PROP)).getEncoder();
